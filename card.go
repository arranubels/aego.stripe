...
func (self *CardClient) SetContext(ctx appengine.Context) {
	self.aectx = ctx
}
...
func (self *CardClient) Create(c *CardParams, customerId string) (*Card, error) {
	...
	err := query("POST", "/v1/customers/"+customerId+"/cards", values, &card, self.aectx)
	return &card, err
}
...
func (self *CardClient) Delete(cardId string, customerId string) (*DeleteResp, error) {
	...
	err := query("DELETE", "/v1/customers/"+customerId+"/cards/"+cardId, values, &delResponse, self.aectx)
